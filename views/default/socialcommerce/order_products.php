<?php
	...
	$order = $vars['entity'];
	if ($order) {
		$order_items = elgg_get_entities_from_relationship('order_item',$order->guid);
		if($order_items){
			$tax_price = 0;
...
EOF;
			...
			$action = $CONFIG->wwwroot.'pg/'.$CONFIG->pluginname.'/'.$_SESSION['user']->username.'/order/';
				...
				<div class="ordered_items">
					<div style="float:right;">
						<form method="post" action="{$action}">
							<input class="order_view_details" type="image" src="{$CONFIG->wwwroot}mod/socialcommerce/images/order_back.gif" value="Back To Order">
						</form>
					</div>
