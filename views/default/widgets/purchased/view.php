...
<?php
	...
	if($display == 1){
		$product = elgg_get_entities("object", "order_item", page_owner(),'', $number);
	}else if($display == 2){
		if ($friends = get_user_friends($user_guid, $subtype, 999999, 0)) {
			...
			$product =  elgg_get_entities('object','order_item','',$friendguids,$number);
		}
	}else if($display == 3){
		$product =  elgg_get_entities("object", "order_item", '', 0, $number);
	}
