...
function SetGeneralValuesInConfig(){
	...
	if(isloggedin()){
		$carts = elgg_get_entities('object','cart',$_SESSION['user']->guid);
		if($carts){
			$items = 0;
			$cart = $carts[0];
	...
	$wishlist_count = elgg_get_entities_from_relationship('wishlist',$_SESSION['user']->guid,false,'','','','','','',true);
	if($wishlist_count){
		$CONFIG->wishlist_item_count = $wishlist_count;
	}
	
	$default_currency = elgg_get_entities_from_metadata('set_default',1,'object','s_currency',0,1);
	if($default_currency){
...
function genarateCartFromSession(){
	...
	if (isloggedin() && isset($_SESSION['GUST_CART']) && !empty($_SESSION['GUST_CART'])) {
		...
		$carts = elgg_get_entities('object','cart',$_SESSION['user']->getGUID());
...
function create_order($user_guid = 0, $CheckoutMethod, $posted_values, $BillingDetails, $ShippingDetails, $ShippingMethod){
	...
	$settings = elgg_get_entities('object','splugin_settings',0,'',9999);
	if($settings){
		$settings = $settings[0];
	}

	$carts = elgg_get_entities('object','cart',$page_owner);
	if($carts){
		...
		if($order_id){
			...
			$cart_items = elgg_get_entities_from_relationship('cart_item',$cart_guid);
			if($cart_items){
				if($ShippingMethod){
					foreach ($cart_items as $cart_item){
...
function calculate_cart_total($cart_user=0,$product_user=0){
	...
	if(isloggedin()){
		...
		$cart = elgg_get_entities('object','cart',$cart_user);
		$cart = $cart[0];
		$cart_items = elgg_get_entities_from_relationship('cart_item',$cart->guid);
	}
