 ...
 **/
	...
	function checkout_payment_settings_paypal(){
		...
		$settings = elgg_get_entities_from_metadata('checkout_method',"{$method}",'object','s_checkout',0,1);
		if($settings){
			$settings = $settings[0];	
		}
	...
	function makepayment_paypal(){
		...
		/*if(!empty($custom)){*/
			...
			$settings = elgg_get_entities_from_metadata('checkout_method','paypal','object','s_checkout',0,1);
			if($settings){
				$settings = $settings[0];	
			}
